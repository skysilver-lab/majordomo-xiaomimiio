...
class XiaomiCloudConnector:
    ...
    def execute_api_call(self, url, params):
        ...
        print ("signed_nonce: " + signed_nonce)
    ...
    def generate_nonce(millis):
        random_num = os.urandom(8);
        print("Random: " + str(random_num));
        nonce_bytes = random_num + (int(millis / 60000)).to_bytes(4, byteorder='big')
...
server = "";
try:
      opts, args = getopt.getopt(sys.argv[1:],"",["server=","username=","password="])
except getopt.GetoptError:
      print ('token_extractor.py --server=<server> --username=<username> --password=<password>')
      sys.exit(2)
for opt, arg in opts:
      if opt in ("--server"):
         server = arg
...
print("Username (email or user ID): " + username)
